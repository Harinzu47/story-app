...
class ShowPassword extends LitWithoutShadowDom {
  ...
  render() {
    return html`
      <input class="form-check-input" id="showPassword" type="checkbox" @click=${() => this._togglePassword()} />
      <label for="showPassword" class="eye-icon">${this.showPassword ? html`<i class="bi-eye-slash"></i>` : html`<i class="bi-eye"></i>`}</label>
    `;
  }
